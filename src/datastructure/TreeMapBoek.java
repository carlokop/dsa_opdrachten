...
public class TreeMapBoek extends AbstractSortedMap<K,V>  {
  ...
  public TreeMapBoek(Comparator<K> comp) {
    super();
    tree.addRoot(null);
  }
  ...
  public V put(K key, V value) throws IllegalArgumentException {
    checkKey(key);
    Entry<K,V> newEntry = MapEntry<>(key,value);
    Position<Entry<K,V>> p = treeSearch(root(),key);
    if(isExternal(p)) {
  ...
  public V remove(K key) thrown IllegalArgumentException {
    checkKey(key);
    Position<Entry<K,V>> p = treeSearch(root(), key);
    if(isExternal(p) ) {
      rebalanceAccess(p);
